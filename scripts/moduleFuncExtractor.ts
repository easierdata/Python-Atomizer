...
class moduleFuncExtractor {
    ...
    async readPrimaryExtraction(): Promise<any> {
        ...
        for (let x = 0; x < inputFiles.length; x++) {
            this.findModulesInvocations(inputFiles[x])
        }
    }

    async createInvocationDictionary(): Promise<any> {
    ...
    async findModulesInvocations(directory: fs.PathLike): Promise<any> {
        ...
        for (let x = 0; x < functionLines.length; x++) {
            for (let i = 0; i < this.invocations.length; i++) {
                ...
                if (functionLines[x].includes(`${invocationName[invocationName.length - 1]}(`) ||
                    ...
                    if (this.primaryFunctionsBlob[`${functionName[functionName.length - 1]}`]) {
                    ...
                    } else if (this.primaryFunctionsBlob[`_${functionName[functionName.length - 1]}`]) {
 ...
 * }
 * 
 * note: weird numpy array behaviour (talk about it during friday meeting)
 */
